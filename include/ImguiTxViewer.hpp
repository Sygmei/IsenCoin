...
#define _PRISizeT   "zX"
        ...
        void DrawWindow(const std::string& title, ic::Transaction* tx, const u8* mem_data, size_t mem_size, bool* close_ptr, size_t base_display_addr = 0x0000)
        {
            Sizes s;
            CalcSizes(s, mem_size, base_display_addr);
            //ImGui::SetNextWindowSizeConstraints(ImVec2(0.0f, 0.0f), ImVec2(s.WindowWidth, FLT_MAX));

            if (ImGui::Begin(title.c_str(), close_ptr, ImGuiWindowFlags_NoScrollbar))
            {
                if (ImGui::IsWindowHovered(ImGuiHoveredFlags_RootAndChildWindows) && ImGui::IsMouseClicked(1))
                    ImGui::OpenPopup("context");
                ImGui::Text(fmt::format("Sender : {}", 
                    base58::encode(tx->get_sender().data(), tx->get_sender().data() + tx->get_sender().size())).c_str());
                ImGui::Text(fmt::format("Receiver : {}",
                    base58::encode(tx->get_receiver().data(), tx->get_receiver().data() + tx->get_receiver().size())).c_str());
        ...
        void DrawContents(const u8* mem_data, size_t mem_size, size_t base_display_addr = 0x0000)
        {
            Sizes s;
            CalcSizes(s, mem_size, base_display_addr);
            ImGuiStyle& style = ImGui::GetStyle();
